...
function startMemoryPrinter
  set -l LOCALWORKDIR "$WORKDIR/$INNERWORKDIR"
  eval ./printMemory.fish $LOCALWORKDIR &
end
...
function startClusterStarter
  ...
  eval $STARTER start --starter.wait --starter.data-dir $DATA_PATH --server.js-dir $JS_PATH --server.arangod $ARANGOD_PATH $ENTERPRISE_JS_PATH $JOIN_PART --server.storage-engine $STORAGE_ENGINE --all.server.maximal-threads 512 --all.dbserver.log.level "rocksdb=DEBUG" --all.dbserver.rocksdb.debug-logging true
