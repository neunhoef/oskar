...
ForEach($file in $(Get-ChildItem -Path $INNERWORKDIR -Filter "ArangoDB3-*.exe").fullName)
{
  Copy-Item "$file" -Destination "release/packages/Community/Windows"
}

ForEach($file in $(Get-ChildItem -Path $INNERWORKDIR -Filter "ArangoDB3-*.zip").fullName)
{
  Copy-Item "$file" -Destination "release/packages/Community/Windows"
}

ForEach($file in $(Get-ChildItem -Path $INNERWORKDIR -Filter "ArangoDB3e-*.exe").fullName)
{
  Copy-Item "$file" -Destination "release/packages/Enterprise/Windows"
}

ForEach($file in $(Get-ChildItem -Path $INNERWORKDIR -Filter "ArangoDB3e-*.zip").fullName)
{
  Copy-Item "$file" -Destination "release/packages/Enterprise/Windows"
}
