...
Function launchSingleTests
{
    ...
    Function test1([array]$test)
    {
        ...
        unittest "$($test[0]) --cluster false --storageEngine $STORAGEENGINE --minPort $portBase --maxPort $($portBase + 99) $($test[2..$($test.Length)]) --skipNonDeterministic true --skipTimeCritical true" -output "$($test[0])_$($test[1]).log"
...
Function launchClusterTests
{
    ...
    Function test3([array]$test)
    {
        ...
        unittest "$($test[0]) --test $($test[2]) --storageEngine $STORAGEENGINE --cluster true --minPort $portBase --maxPort $($portBase + 99) --skipNonDeterministic true" -output "$($test[0])_$($test[1]).log"
        $portBase = $($portBase + 100)
        Start-Sleep 5
    }
    $UPIDS = $null
