...
$arguments = @("choco install -y dotnet4.7.1 vcredist-all cmake.portable nsis python2 procdump windbg wget nuget.commandline vim notepadplusplus putty.install openssh","choco install -y git winflexbison3 ruby","choco install -y ruby2.devkit nodejs jdk8 strawberryperl nasm","gem install bundler persistent_httparty rspec rspec-core","npm install -g gitbook-cli","pip3.5 install git+https://github.com/frerich/clcache.git")
